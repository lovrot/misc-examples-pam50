...
```{r}
eset <- genefilter::nsFilter(mainz)$eset
...
```{r, fig.height = 4.5, fig.cap = "Figure. t-SNE plots with median-centered data and using one minus Spearman correlation distance metric. (A) After non-specific gene-filtering, and (B) based on the fifty PAM50 genes. t-SNE: t-distributed stochastic neighbour embedding."}
...
```{r fig2, fig.cap = "Figure. Excess distant metastases in relation to PAM50 (A) intrinsic subtype and (B) proliferation index."}
...
```{r fig3, fig.width = 9, fig.height = 6, fig.cap = "Figure. Corresponding Kaplan-Meier curves. The PAM50 proliferation index is categorised into quarters."}
...
```{r fig4, fig.height = 10, fig.cap = "Figure. Proliferation, intrinsic subtypes and outcome."}
ylim0 <- range(mainz$excessdm)
...
```{r fig6, fig.cap = "Figure. Concordance indices to assess the added value of ROR-S beyond standard clinical-pathological prognosticators as represented by the Nottingham prognostic index (NPI) in (A) ER+ patients and (B) ER+/not H2 patients."}
...
Copyright &copy; 2015-2017 by John Lövrot.
